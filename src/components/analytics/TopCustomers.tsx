...
export function TopCustomers({ data, loading }: TopCustomersProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#d2d2d7]/30 p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[15px] font-semibold text-[#1d1d1f]">Top Customers</h3>
        <Link 
      ...
      {data.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-8 text-center">
          ...
          <p className="text-[13px] text-[#86868b]">No customer data yet</p>
        </div>
      ) : (
        <div className="space-y-3">
          {data.slice(0, 5).map((customer, i) => {
            const percentage = total > 0 ? (customer.value / total) * 100 : 0
            return (
