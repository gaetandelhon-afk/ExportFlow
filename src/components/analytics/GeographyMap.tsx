  ...
  return (
    <div className="bg-white rounded-2xl border border-[#d2d2d7]/30 p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[15px] font-semibold text-[#1d1d1f]">Sales by Country</h3>
        <Link 
          href="/reports/geography"
      ...
      {data.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-8 text-center">
          ...
          <p className="text-[13px] text-[#86868b]">No geographic data yet</p>
        </div>
      ) : (
        <div className="space-y-3">
          {data.slice(0, 6).map((country, i) => {
            const percentage = total > 0 ? (country.value / total) * 100 : 0
            const countryCode = String(country.dimension)
            return (
              <div key={i}>
