...
export default function NewOrderPage() {
  ...
  const [inputCartQuantities, setInputCartQuantities] = useState<Record<string, string>>({})

  const setCartQuantity = (cartKey: string, quantity: number) => {
    const validQty = Math.max(1, quantity)
    setCart(prev => prev.map(item =>
      item.cartKey === cartKey ? { ...item, quantity: validQty } : item
    ))
    setInputCartQuantities(prev => ({ ...prev, [cartKey]: String(validQty) }))
  }
  const getCartInputQty = (cartKey: string, currentQty: number) =>
    inputCartQuantities[cartKey] ?? String(currentQty)
  const handleCartQtyInputChange = (cartKey: string, raw: string) => {
    setInputCartQuantities(prev => ({ ...prev, [cartKey]: raw }))
    const val = parseInt(raw, 10)
    if (!isNaN(val) && val >= 1) {
      setCart(prev => prev.map(item =>
        item.cartKey === cartKey ? { ...item, quantity: val } : item
      ))
    }
  }
  const handleCartQtyInputBlur = (cartKey: string, currentQty: number) => {
    const val = parseInt(inputCartQuantities[cartKey] || '', 10)
    const validQty = isNaN(val) || val < 1 ? currentQty : val
    setCart(prev => prev.map(item =>
      item.cartKey === cartKey ? { ...item, quantity: validQty } : item
    ))
    setInputCartQuantities(prev => ({ ...prev, [cartKey]: String(validQty) }))
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      // Admin-created orders use CNY because catalog prices are in CNY (RMB)
      // The currency is NOT converted - values stay as-is in CNY
      const orderCurrency = 'CNY'
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="h-14 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <Link href="/orders" className="text-[#0071e3] hover:text-[#0077ed] flex items-center gap-1 text-[13px]">
                <ChevronLeft className="w-4 h-4" />
                <span className="hidden sm:inline">Orders</span>
              </Link>
              <h1 className="text-[17px] font-semibold text-[#1d1d1f]">New Order</h1>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <div className="flex gap-6">
          ...
          <main className="flex-1 min-w-0">
            {/* Filters Bar - Same as products page */}
            ...
            {/* Categories Row - Same as products page */}
            {categories.length > 0 && (
                ...
                {filteredProducts.map((product) => {
                  ...
                  if (viewMode === 'list') {
                    return (
                      ...
                      >
                        {/* Image - same size as products page list view */}
                          ...
                          {inCart ? (
                            ...
                            <div className="flex items-center rounded-lg overflow-hidden bg-[#0071e3]/5" style={{ border: '1px solid rgba(0, 113, 227, 0.3)' }}>
                              ...
                              <input
                                type="text"
                                inputMode="numeric"
                                value={getCartInputQty(inCart.cartKey, inCart.quantity)}
                                onChange={(e) => handleCartQtyInputChange(inCart.cartKey, e.target.value)}
                                onBlur={() => handleCartQtyInputBlur(inCart.cartKey, inCart.quantity)}
                                onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleCartQtyInputBlur(inCart.cartKey, inCart.quantity) } }}
                  ...
                  // Grid Views - matching products page exactly
                  return (
                    <div
                      key={product.id}
                    ...
                    >
                      ...
                      {/* Product Image - exact same sizing as products page */}
                      ...
                      {/* Product Info - exact same sizing as products page */}
                      <div className={`${viewMode === 'grid-small' ? 'p-2' : 'p-4'} flex flex-col`}>
                        ...
                        {/* Chinese name - only on larger views */}
                        ...
                        {/* Quantity & Add to Cart — stepper dans toutes les vues, taille adaptée */}
                        <div className={`flex items-center gap-2 mt-auto ${viewMode === 'grid-small' ? 'pt-2' : 'pt-3'}`}>
                          <div 
                          ...
                          >
                            <button
                              onClick={() => setProductQuantity(product.id, qty - 1)}
                              className={`flex items-center justify-center hover:bg-gray-100 transition-colors flex-shrink-0 ${
                                viewMode === 'grid-small' ? 'w-5 h-6' : viewMode === 'grid-medium' ? 'w-6 h-7' : 'w-8 h-8'
                              }`}
                              disabled={qty <= 1}
                            >
                              <Minus className={`text-[#86868b] ${viewMode === 'grid-small' ? 'w-2 h-2' : 'w-3 h-3'}`} />
                            </button>
                            <input
                              type="text"
                              inputMode="numeric"
                              ...
                              className={`text-center font-medium bg-transparent focus:outline-none flex-1 min-w-0 ${
                                viewMode === 'grid-small' ? 'text-[10px] h-6' : viewMode === 'grid-medium' ? 'text-[11px] h-7' : 'text-[12px] h-8'
                              }`}
                            />
                            <button
                              onClick={() => setProductQuantity(product.id, qty + 1)}
                            ...
                            >
                              <Plus className={`text-[#86868b] ${viewMode === 'grid-small' ? 'w-2 h-2' : 'w-3 h-3'}`} />
                            </button>
                          </div>
                          <button
                            onClick={() => handleProductClick(product)}
                            className={`bg-[#0071e3] text-white rounded-lg flex items-center justify-center gap-1 font-medium hover:bg-[#0077ed] transition-colors flex-shrink-0 ${
                              viewMode === 'grid-small' ? 'h-6 px-1.5 text-[10px]' : viewMode === 'grid-medium' ? 'h-7 px-2 text-[11px]' : 'h-8 px-3 text-[12px]'
                            }`}
                          >
                            {hasOptions ? (
                              <Settings2 className={viewMode === 'grid-small' ? 'w-2.5 h-2.5' : 'w-3.5 h-3.5'} />
                            ) : (
                              <ShoppingCart className={viewMode === 'grid-small' ? 'w-2.5 h-2.5' : 'w-3.5 h-3.5'} />
                            )}
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-[#d2d2d7]/30 sticky top-20 overflow-hidden">
              ...
              <div className="max-h-[45vh] overflow-y-auto">
                {cart.length === 0 ? (
                  <div className="p-8 text-center">
                    <ShoppingCart className="w-10 h-10 text-[#d2d2d7] mx-auto mb-2" />
                    <p className="text-[14px] text-[#86868b]">Your cart is empty</p>
                  </div>
                ) : (
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <div className="flex items-center gap-1">
                            ...
                            <input
                              type="text"
                              inputMode="numeric"
                              value={getCartInputQty(item.cartKey, item.quantity)}
                              onChange={(e) => handleCartQtyInputChange(item.cartKey, e.target.value)}
                              onBlur={() => handleCartQtyInputBlur(item.cartKey, item.quantity)}
                              onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleCartQtyInputBlur(item.cartKey, item.quantity) } }}
              ...
              <div className="p-4 border-t border-[#d2d2d7]/30 bg-[#f5f5f7]">
                ...
                <button
                  ...
                  className="w-full h-11 flex items-center justify-center bg-[#0071e3] text-white text-[14px] font-medium rounded-xl disabled:bg-[#d2d2d7] hover:bg-[#0077ed] transition-colors"
                >
          ...
          <div className="fixed inset-0 bg-black/50 z-50 lg:hidden" onClick={() => setShowMobileCart(false)}>
            <div className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-white" onClick={(e) => e.stopPropagation()}>
              <form onSubmit={handleSubmit} className="h-full flex flex-col">
                <div className="flex items-center justify-between p-4 border-b border-[#d2d2d7]/30">
                  <h2 className="text-[17px] font-semibold text-[#1d1d1f]">Cart ({cartItemCount})</h2>
                  <button type="button" onClick={() => setShowMobileCart(false)} className="p-2"><X className="w-5 h-5" /></button>
                </div>
                <div className="flex-1 overflow-y-auto p-4">
                  {cart.length === 0 ? (
                    <div className="text-center py-12"><ShoppingCart className="w-12 h-12 text-[#d2d2d7] mx-auto mb-3" /><p className="text-[15px] text-[#86868b]">Your cart is empty</p></div>
                  ) : (
                        ...
                        <div key={item.cartKey} className="bg-[#f5f5f7] rounded-xl p-3">
                          ...
                          <div className="flex items-center justify-between mt-3">
                            <div className="flex items-center gap-2">
                              <button type="button" onClick={() => updateCartQuantity(item.cartKey, -1)} className="w-8 h-8 bg-white rounded-lg flex items-center justify-center"><Minus className="w-4 h-4" /></button>
                              <input
                                type="text"
                                inputMode="numeric"
                                value={getCartInputQty(item.cartKey, item.quantity)}
                ...
                <div className="border-t border-[#d2d2d7]/30 p-4 bg-[#f5f5f7]">
                  <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} placeholder="Order notes..." className="w-full px-3 py-2 bg-white rounded-lg text-[14px] focus:outline-none focus:ring-2 focus:ring-[#0071e3] resize-none mb-3" />
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-6 py-3">
          ...
          {(error || (!selectedCustomer && cart.length > 0)) && (
            <div className="mt-2 flex flex-wrap gap-2">
              {error && (
                <span className="lg:hidden text-[12px] text-[#ff3b30] bg-[#ff3b30]/10 px-2 py-1 rounded">{error}</span>
              )}
              {!selectedCustomer && cart.length > 0 && (
                <span className="text-[12px] text-[#ff9500] flex items-center gap-1">
                  <User className="w-3 h-3" />
                  Select a customer to create the order
                </span>
              )}
            </div>
          )}
