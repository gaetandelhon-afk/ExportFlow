...
import { 
  ...
  Calendar, CreditCard, Package, Edit3, Plus, Trash2,
  Save, X, DollarSign
} from 'lucide-react'
...
interface Invoice {
  ...
  type: string // INVOICE, QUOTE, PROFORMA
  status: string
  issueDate: string
  dueDate: string | null
...
export default function InvoiceDetailPage() {
  ...
  const [invoiceSerials, setInvoiceSerials] = useState<{ orderLineId: string; serial: string }[]>([])
  ...
  useEffect(() => {
    ...
    const loadTemplates = async () => {
      const invoiceTemplates = await getTemplatesForType('invoice')
  ...
  const fetchInvoice = async () => {
    try {
      ...
      const res = await authFetch(`/api/invoices/${invoiceId}?t=${Date.now()}`, {
        cache: 'no-store'
      })
      if (!res.ok) {
      ...
      console.log('Fetched invoice data:', { totalAmount: data.invoice?.totalAmount, discounts: data.invoice?.order?.discounts })
      setInvoice(data.invoice)
      setInvoiceSerials(data.serials || [])
    ...
    try {
      const res = await authFetch(`/api/invoices/${invoiceId}`, {
        ...
        body: JSON.stringify({
          ...
          createdAt: editForm.issueDate ? new Date(editForm.issueDate).toISOString() : undefined,
  ...
  const getDefaultEmailBody = () => {
    if (!invoice) return ''
    const customerName = invoice.order.customer.contactName || invoice.order.customer.companyName
    const dueDate = invoice.dueDate ? formatDate(invoice.dueDate) : '30 days from issue date'
    const docType = invoice.type === 'PROFORMA' ? 'proforma invoice' : 'invoice'
    return `Dear ${customerName},

Please find attached our ${docType} ${invoice.invoiceNumber} for your review.
...
Best regards`
    ...
    if (template) {
      ...
      try {
        ...
        if (template.settings.layoutElements) {
          settings.invoiceLayout = template.settings.layoutElements
        }
        if (template.settings.tableColumns) {
          settings.invoiceTableColumns = template.settings.tableColumns
        }
        ...
        localStorage.setItem(`invoice_template_${invoiceId}`, templateId)
  ...
  const handleDownloadPdf = () => {
    ...
    generateAdminInvoicePdf({
      ...
      order: {
        ...
        lines: invoice.order.lines.map(line => {
          const lineSerials = invoiceSerials.filter(s => s.orderLineId === line.id).map(s => s.serial)
          return {
        ...
        charges: invoice.order.charges?.map(c => ({
          description: c.description,
          amount: Number(c.amount),
        })) || [],
        discounts: invoice.order.discounts?.map(d => ({
      ...
      customer: {
        ...
        contactName: invoice.order.customer.contactName,
        billingAddress: invoice.order.customer.billingAddress,
        vatNumber: invoice.order.customer.vatNumber,
      }
    })
  }
  ...
  // Edit mode functions
  const startEditing = () => {
    console.log('Starting edit, invoice.order.discounts:', invoice?.order.discounts)
    setEditCharges(invoice?.order.charges?.map(c => ({
      id: c.id,
      description: c.description,
      amount: Number(c.amount)
    })) || [])
    // Load existing discounts
    const loadedDiscounts = invoice?.order.discounts?.map(d => ({
      id: d.id,
      description: d.description,
      type: d.type as 'fixed' | 'percent_products' | 'percent_total',
      value: Number(d.value)
    })) || []
    console.log('Loaded discounts:', loadedDiscounts)
    setEditDiscounts(loadedDiscounts)
    setIsEditing(true)
  }
  ...
  const calculateDiscountAmount = (discount: { type: string; value: number }) => {
    // Calculate subtotal from actual line totals (not from stored invoice.subtotal)
    const subtotal = invoice 
      ? invoice.order.lines.reduce((sum, line) => sum + Number(line.lineTotal), 0)
      : 0
  ...
  const saveCharges = async () => {
    ...
    try {
      ...
      const discountsRes = await authFetch(`/api/orders/${invoice.order.id}/discounts`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ discounts: discountsWithAmounts, invoiceId: invoice.id })
      })
      
      const discountsData = await discountsRes.json()
      console.log('Discounts response:', discountsData)

      // Save charges scoped to this document
      const chargesRes = await authFetch(`/api/orders/${invoice.order.id}/charges`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ charges: validCharges, invoiceId: invoice.id })
      })
  ...
  // Calculate totals dynamically (never rely on stale stored totalAmount)
  const editSubtotal = invoice 
    ? invoice.order.lines.reduce((sum, line) => sum + Number(line.lineTotal), 0)
    : 0
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          ...
          <p className="text-[15px] text-[#86868b]">{error || 'Invoice not found'}</p>
          <Link href="/invoices" className="text-[#0071e3] text-[14px] mt-4 inline-block">
            Back to Invoices
          </Link>
        </div>
      </div>
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between mb-4">
          <Link href="/invoices" className="text-[#0071e3] hover:text-[#0077ed] flex items-center gap-1 text-[13px]">
          ...
          {(invoice.status === 'sent' || invoice.status === 'overdue') && (
            <button
              ...
              disabled={updating}
              className="inline-flex items-center gap-2 h-10 px-4 bg-[#34c759] hover:bg-[#2db350] text-white text-[13px] font-medium rounded-xl transition-colors disabled:opacity-50 whitespace-nowrap flex-shrink-0"
            >
          ...
          {invoice.status === 'paid' && (
            <button
              ...
              disabled={updating}
              className="inline-flex items-center gap-2 h-10 px-4 bg-[#ff9500] hover:bg-[#e6860a] text-white text-[13px] font-medium rounded-xl transition-colors disabled:opacity-50 whitespace-nowrap flex-shrink-0"
            >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 h-10 px-4 bg-[#0071e3] hover:bg-[#0077ed] text-white text-[13px] font-medium rounded-xl transition-colors"
          >
          ...
          <DocumentExportDropdown
            data={{
              ...
              issueDate: invoice.issueDate,
              dueDate: invoice.dueDate,
              customerName: invoice.order?.customer?.companyName,
              currency: invoice.currency,
              subtotal: invoice.subtotal,
              totalCharges: invoice.order?.charges?.reduce((sum, c) => sum + Number(c.amount), 0) || 0,
              totalAmount: invoice.totalAmount,
              lines: invoice.order?.lines?.map(l => ({
                ref: l.product?.ref,
        ...
        <div className="mt-2">
          <div className="flex items-center gap-2">
            <h1 className="text-[28px] font-semibold text-[#1d1d1f] whitespace-nowrap">{invoice.invoiceNumber}</h1>
            <button
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <span className="text-[14px] text-[#86868b]">
              Order: <Link href={`/orders/${invoice.order.id}`} className="text-[#0071e3] hover:underline">{invoice.order.orderNumber}</Link>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#d2d2d7]/30 overflow-hidden print:border-0 print:shadow-none">
        ...
        <div className="p-8 border-b border-[#d2d2d7]/30">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              <p className="text-[24px] font-bold text-[#1d1d1f] mb-2">
                {invoice.type === 'QUOTE' ? 'QUOTE' : invoice.type === 'PROFORMA' ? 'PROFORMA INVOICE' : 'INVOICE'}
              </p>
              <p className="text-[15px] font-medium text-[#1d1d1f]">{invoice.invoiceNumber}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 grid grid-cols-2 gap-8 border-b border-[#d2d2d7]/30">
          <div>
            <p className="text-[11px] uppercase tracking-wide text-[#86868b] font-medium mb-2">Bill To</p>
            <p className="text-[15px] font-semibold text-[#1d1d1f]">{invoice.order.customer.companyName}</p>
            {invoice.order.customer.contactName && (
              <p className="text-[14px] text-[#1d1d1f]">{invoice.order.customer.contactName}</p>
            )}
            {invoice.order.customer.billingAddress && (
              <p className="text-[14px] text-[#86868b] whitespace-pre-line mt-1">
                {invoice.order.customer.billingAddress}
              </p>
            )}
            {invoice.order.customer.vatNumber && (
              <p className="text-[13px] text-[#86868b] mt-2">VAT: {invoice.order.customer.vatNumber}</p>
            )}
          </div>
          <div className="text-right">
            <div className="space-y-2">
              <div className="flex justify-end items-center gap-2">
                ...
                <span className="text-[14px] font-medium text-[#1d1d1f]">{formatDate(invoice.issueDate)}</span>
              </div>
              {invoice.dueDate && (
                <div className="flex justify-end items-center gap-2">
                  <CreditCard className="w-4 h-4 text-[#86868b]" />
                  <span className="text-[13px] text-[#86868b]">Due Date:</span>
                  <span className="text-[14px] font-medium text-[#1d1d1f]">{formatDate(invoice.dueDate)}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="p-8">
                  ...
                  <td className="py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {invoiceSerials.filter(s => s.orderLineId === line.id).length > 0 && (
                          <p className="text-[11px] text-[#86868b] mt-0.5 font-mono">
                            S/N: {invoiceSerials.filter(s => s.orderLineId === line.id).map(s => s.serial).join(', ')}
                          </p>
                        )}
                      </div>
                    </div>
          ...
          {!isEditing && invoice.order.charges && invoice.order.charges.length > 0 && (
            <div className="mt-4 pt-4 border-t border-[#d2d2d7]/30">
              ...
              {invoice.order.charges.map((charge) => (
          ...
          {/* Discounts - View Mode */}
          {!isEditing && invoice.order.discounts && invoice.order.discounts.length > 0 && (
            <div className="mt-4 pt-4 border-t border-[#d2d2d7]/30">
              <p className="text-[11px] uppercase tracking-wide text-[#34c759] font-medium mb-3">Discounts Applied</p>
              {invoice.order.discounts.map((discount) => (
              ...
              <div className="flex justify-end">
                <div className="w-64 space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="text-[14px] text-[#1d1d1f]">{currencySymbol}{formatNumber(Number(invoice.subtotal))}</span>
                  </div>
                  {invoice.order.charges && invoice.order.charges.length > 0 && (
                    <div className="flex justify-between">
                      <span className="text-[14px] text-[#86868b]">Charges</span>
                      <span className="text-[14px] text-[#1d1d1f]">
                        + {currencySymbol}{formatNumber(invoice.order.charges.reduce((sum, c) => sum + Number(c.amount), 0))}
                      </span>
                    </div>
                  )}
                  {invoice.order.discounts && invoice.order.discounts.length > 0 && (
                    <div className="flex justify-between">
                      <span className="text-[14px] text-[#34c759]">Discounts</span>
                      <span className="text-[14px] text-[#34c759]">
                        - {currencySymbol}{formatNumber(invoice.order.discounts.reduce((sum, d) => sum + Number(d.amount), 0))}
                      </span>
                    </div>
                  )}
                  <div className="flex justify-between pt-2 border-t border-[#d2d2d7]/30">
                    ...
                    <span className="text-[20px] font-bold text-[#0071e3]">{currencySymbol}{formatNumber(Number(invoice.totalAmount))}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 m-4">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-[17px] font-semibold text-[#1d1d1f]">Edit Invoice Details</h2>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-[13px] font-medium text-[#86868b] mb-1.5">Invoice Number</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[13px] font-medium text-[#86868b] mb-1.5">Due Date</label>
                <input
                  type="date"
      ...
      <EmailModal
        ...
        documentType={invoice?.type === 'PROFORMA' ? 'Proforma' : 'Invoice'}
        documentNumber={invoice?.invoiceNumber || ''}
        defaultTo={invoice?.order.customer.email}
        defaultSubject={invoice ? `${invoice.type === 'PROFORMA' ? 'Proforma Invoice' : 'Invoice'} ${invoice.invoiceNumber} - ${invoice.order.customer.companyName}` : ''}
